...
def readColmapCameras(cam_extrinsics, cam_intrinsics, depths_params, images_folder, depths_folder, test_cam_names_list, used_in_train=None):
    cam_infos = []
    test_cam_name_set = set(os.path.basename(name) for name in test_cam_names_list)
    if used_in_train:
        cam_extrinsics = {key: value for key, value in cam_extrinsics.items() if value.name not in used_in_train}
...
def readColmapSceneInfo(
    ...
    if cldm_dataset_path:
        ...
        cldm_cam_infos_unsorted = readColmapCameras(
            ...
            cam_intrinsics=cldm_cam_intrinsics,
            depths_params=None,
